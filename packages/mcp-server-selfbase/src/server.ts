...
import type { SelfbasePlatform } from './platform/types.js';
...
export type SelfbaseMcpServerOptions = {
  /**
   * Platform implementation for Selfbase.
   */
  platform: SelfbasePlatform;
  ...
  /**
   * Callback for after a selfbase tool is called.
   */
...
/**
 * Creates an MCP server for interacting with self-hosted Selfbase.
 */
export function createSelfbaseMcpServer(options: SelfbaseMcpServerOptions) {
  const {
    platform,
    projectId = 'default',
    readOnly,
  ...
  const server = createMcpServer({
    name: 'selfbase-mcp-sf',
    title: 'Selfbase MCP (Self-Hosted)',
    version,
