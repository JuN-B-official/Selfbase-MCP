...
export type SelfHostedPlatformOptions = {
    /**
     * The URL for the self-hosted Selfbase instance.
     * Example: http://localhost or https://selfbase.yourdomain.com
     */
    selfbaseUrl: string;

    /**
     * The service role key for the self-hosted Selfbase instance.
...
/**
 * Creates a Selfbase platform implementation for self-hosted environments.
 * This bypasses the Selfbase Management API and connects directly to the services.
 */
export function createSelfHostedPlatform(
    options: SelfHostedPlatformOptions
): SelfbasePlatform {
    const { selfbaseUrl, serviceRoleKey, anonKey, postgresUrl } = options;

    // Normalize URL (remove trailing slash)
    const baseUrl = selfbaseUrl.replace(/\/$/, '');
        ...
        try {
            ...
            if (response.status === 404) {
                throw new Error(
                    ...
                    `3. Check pg-meta logs: docker logs selfbase-meta`
                );
            }
            ...
            throw new Error(
                ...
                `Troubleshooting:\n` +
                `1. Check if pg-meta service is running: docker ps | grep meta\n` +
                `2. Check pg-meta logs: docker logs selfbase-meta\n` +
                `3. Restart Selfbase: docker compose restart meta`
            );
        }
    }
    ...
    const database: DatabaseOperations = {
        ...
        async listMigrations(projectId: string): Promise<Migration[]> {
            // Query the selfbase_migrations schema
            const query = `
        SELECT version, name 
        FROM selfbase_migrations.schema_migrations 
      ...
      `;
        ...
        async applyMigration(projectId: string, options: ApplyMigrationOptions): Promise<void> {
            ...
            const insertQuery = `
        INSERT INTO selfbase_migrations.schema_migrations (version, name)
        VALUES ('${version}', '${name}')
      `;
    ...
    const debugging: DebuggingOperations = {
        async getLogs(projectId: string, options: GetLogsOptions): Promise<unknown> {
            ...
            const containerMap: Record<string, string> = {
                api: 'selfbase-kong',
                postgres: 'selfbase-db',
                auth: 'selfbase-auth',
                storage: 'selfbase-storage',
                realtime: 'selfbase-realtime',
                functions: 'selfbase-functions',
            };
            ...
            try {
                ...
                if (!response.ok) {
                    const containerName = containerMap[service] || `selfbase-${service}`;
                    return {
                ...
                if (!contentType?.includes('application/json')) {
                    ...
                    return {
                        ...
                        suggestion: `Check Docker logs directly: docker logs ${containerName}`,
                        note: 'Self-hosted Selfbase requires explicit analytics/logflare setup'
                    };
                }
...
Available at: ${baseUrl}/functions/v1/${name}`,
    ...
    // Docs operations - search Selfbase documentation
    const docs: DocsOperations = {
        async searchDocs(query: string): Promise<DocsSearchResult[]> {
            // Search Selfbase docs via their search API or provide embedded knowledge
            try {
                // Try Selfbase docs search API
                const response = await fetch(`https://selfbase.com/docs/api/search?q=${encodeURIComponent(query)}`, {
                    method: 'GET',
...
docker exec selfbase-db pg_dumpall -U postgres > backup_${timestamp}.sql

Or use the backup script:
./scripts/backup.sh ${options?.output_path || ''}

To restore:
docker exec -i selfbase-db psql -U postgres < backup_${timestamp}.sql`,
            };
        },
        ...
        async init(info: InitData) {
            try {
                ...
                if (!response.ok && response.status !== 404) {
                    throw new Error(`Failed to connect to Selfbase at ${baseUrl}`);
                }
            } catch (error) {
                console.error('Warning: Could not verify connection to Selfbase:', error);
            }
        },
        database,
        debugging,
...
// Embedded documentation for common Selfbase topics
function getEmbeddedDocs(query: string): DocsSearchResult[] {
    ...
    if (q.includes('auth') || q.includes('login') || q.includes('signup')) {
        results.push({
            ...
            url: 'https://selfbase.com/docs/guides/auth',
            content: 'Selfbase Auth provides user authentication with email/password, magic links, OAuth providers (Google, GitHub, etc.), and phone auth.',
            section: 'Auth',
        });
    }
    ...
    if (q.includes('storage') || q.includes('file') || q.includes('upload')) {
        results.push({
            title: 'Storage',
            url: 'https://selfbase.com/docs/guides/storage',
            content: 'Selfbase Storage allows you to store and serve files. Create buckets, upload files, and generate signed URLs for secure access.',
            section: 'Storage',
        });
    }
    ...
    if (q.includes('database') || q.includes('sql') || q.includes('postgres')) {
        results.push({
            title: 'Database',
            url: 'https://selfbase.com/docs/guides/database',
            content: 'Selfbase uses PostgreSQL with extensions like pgvector for AI embeddings. Use Row Level Security (RLS) for data protection.',
            section: 'Database',
        });
    }
    ...
    if (q.includes('edge') || q.includes('function') || q.includes('serverless')) {
        results.push({
            ...
            url: 'https://selfbase.com/docs/guides/functions',
    ...
    if (q.includes('realtime') || q.includes('subscription') || q.includes('websocket')) {
        results.push({
            title: 'Realtime',
            url: 'https://selfbase.com/docs/guides/realtime',
            content: 'Selfbase Realtime enables live database changes via WebSocket. Subscribe to INSERT, UPDATE, DELETE events on tables.',
            section: 'Realtime',
        });
    }
    ...
    if (q.includes('rls') || q.includes('security') || q.includes('policy')) {
        results.push({
            ...
            url: 'https://selfbase.com/docs/guides/auth/row-level-security',
    ...
    if (results.length === 0) {
        results.push({
            title: 'Selfbase Documentation',
            url: 'https://selfbase.com/docs',
            content: `Search for "${query}" in the Selfbase documentation for detailed guides and API references.`,
            section: 'General',
        });
    }
