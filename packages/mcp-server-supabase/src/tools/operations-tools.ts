...
export function getOperationsTools({ operations, projectId, readOnly }: OperationsToolsOptions) {
    ...
    const operationsTools = {
        check_health: injectableTool({
            description: `Performs a comprehensive health check of the Selfbase instance.
Checks:
...
docker exec -i selfbase-db psql -U postgres < backup_file.sql`,
            annotations: {
                title: 'Backup Now',
                readOnlyHint: false,
                destructiveHint: false,
...
WARNING: This is a critical operation. Use dry_run=true first to preview changes.`,
        ...
        get_stats: injectableTool({
            description: `Gets system statistics and metrics for the Selfbase instance.
Includes:
