...
import { createSelfbaseMcpServer } from '../server.js';
...
async function main() {
    const {
        ...
        options: {
            ['selfbase-url']: {
                type: 'string',
                description: 'Self-hosted Selfbase URL (e.g., http://localhost or https://selfbase.example.com)',
            },
            ['service-role-key']: {
                type: 'string',
    ...
    const selfbaseUrl = cliSelfbaseUrl ?? process.env.SUPABASE_URL;
    ...
    if (!selfbaseUrl) {
        console.error(
            'Please provide the Selfbase URL with --selfbase-url flag or set the SUPABASE_URL environment variable'
        );
        process.exit(1);
    }
    ...
    console.error(`Starting Selfbase MCP Server (self-hosted mode) v${version}`);
    console.error(`Connecting to: ${selfbaseUrl}`);
