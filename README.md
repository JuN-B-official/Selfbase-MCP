# Selfbase MCP Server (Self-Hosted)

> MCP Server for Self-Hosted Selfbase - Optimized for single-instance management

This is a one-person project, built and maintained in spare time.

[![npm version](https://img.shields.io/npm/v/@jun-b/selfbase-mcp)](https://www.npmjs.com/package/@jun-b/selfbase-mcp)
...
Connect AI assistants (Claude, Cursor, etc.) directly to your self-hosted Selfbase instance via the [Model Context Protocol](https://modelcontextprotocol.io/introduction).

# Related projects

- [Selfbase](https://github.com/JuN-B-official/Selfbase)

## Key Features

- **Self-Hosted First** - Designed specifically for self-hosted Selfbase instances
...
| Feature                    | Official Supabase MCP |    Selfbase MCP (Self-Hosted)    |
| -------------------------- | :-------------------: | :------------------------------: |
...
```bash
# Build
docker build -t selfbase .

# Run
docker run -e SELFBASE_URL=http://host.docker.internal:8000 \
           -e SELFBASE_SERVICE_ROLE_KEY=your-key \
           selfbase-mcp
```
...
```json
{
  "mcpServers": {
    "selfbase": {
      "command": "npx",
      "args": ["-y", "@jun-b/selfbase-mcp@latest"],
      "env": {
        "SELFBASE_URL": <URL>,
        "SELFBASE_SERVICE_ROLE_KEY": "your-service-role-key",
        "SELFBASE_ANON_KEY": "your-anon-key"
      }
    }
  }
}
...
| ----------------------------- | -------- | ------------------------ |
| `SELFBASE_URL`              | ✅       | Self-hosted Selfbase URL |
| `SELFBASE_SERVICE_ROLE_KEY` | ✅       | Service role key         |
| `SELFBASE_ANON_KEY`         | ❌       | Anonymous key (optional) |
...
- `search_docs`: Search Selfbase official documentation
...
This enables **AI-powered incident response** for your self-hosted Selfbase!
...
selfbase --features database,operations

# All features
selfbase --features database,debugging,development,storage,auth,functions,branching,docs,operations
```
...
```json
{
  "mcpServers": {
    "selfbase-sf": {
      "command": "npx",
      "args": ["-y", "@jun-b/selfbase-mcp@latest", "--read-only"],
      "env": {
        "SELFBASE_URL": "<URL>",
        "SELFBASE_SERVICE_ROLE_KEY": "your-service-role-key"
      }
...
selfbase --features database,operations,storage,auth

# Staging (monitoring only)
selfbase --read-only --features database,debugging,docs

# Production (minimal, safe access)
selfbase --read-only --features database,docs
```

### 6. Security Checklist

Before deploying AI access to your Selfbase instance:
...
```
"Check the health of my Selfbase instance. If everything is healthy, 
...
```
"Give me a summary of my Selfbase instance: database size, active connections, 
total users, and storage usage."
```

## Related Projects

- **[selfbase-sf](https://github.com/Song-JunHyeong/selfbase-sf)** - Production-ready Docker Compose setup for self-hosting Selfbase with automated secret management. This MCP server is designed to work seamlessly with selfbase-sf.
